/**
 * Created by agust_000 on 2014/11/10.
 */
/**
 * 基于jquery的瀑布流显示图片的组件
 */
...
//        var itemHeight = Math.ceil(Math.ceil(imgWidth) * imageHeight / imageWidth) + options.offsetV;
        ...
        // 重新计算被插入列的高度
        columns[minColumn] += itemElem.offsetHeight + options.margin;
        ...
        // 把图片数据push到waters中
        pushWater(item, itemElem.offsetWidth, itemElem.offsetHeight);
    };
...
//        itemElem.style.height = 'auto';
        itemElem.style.top = itemTop + 'px';
        itemElem.style.left = itemLeft + 'px';
//console.log(itemElem.offsetWidth);
        columns[minColumn] += itemElem.offsetHeight + options.margin;
        calculateColumnsHeight();
    };
...
//        var thisPushWater = pushWater;
        ...
        for(var i = 0; i < pageCount; i++){
            ...
            img = new Image();
            img.src = url;
            $(img).data('data', JSON.stringify(item));
